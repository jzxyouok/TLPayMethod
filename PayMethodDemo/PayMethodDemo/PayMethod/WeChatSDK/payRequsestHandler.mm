...
//初始化函数
-(instancetype) init:(NSString *)app_id mch_id:(NSString *)mch_id
{
    //初始构造函数
...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_demoOrderName:(NSString*)orderName AndOrderPrice:(NSString*)orderPrice NotifyURL:(NSString*)notifyURL Spbill_create_ip:(NSString*)spbill_create_ip OrderID:(NSString*)orderID Attach:(NSString *)attach
{
   ...
   // [packageParams setObject: @"APP-001"        forKey:@"device_info"]; //支付设备号或门店号
    ...
    [packageParams setObject: [NSString stringWithFormat:@"%@",orderID]           forKey:@"out_trade_no"];//商户订单号
    ...
    [packageParams setObject: attach             forKey:@"attach"];          // 其他的数据
